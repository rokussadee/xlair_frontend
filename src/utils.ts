...
//        const startTime = timeElement.textContent
//          ?.trim()
//          .split("–")[0]
//          .trim();
//        const endTime = timeElement
...
//          ".title"
//        ) as HTMLElement;
//        const descriptionElement = eventElement.querySelector(
//          ".eventdesc"
//        ) as HTMLElement;
//
...
export const fetchPageContent = async (
  url: string
): Promise<string | InvalidResponse> => {
  try {
...
export const parseCalendarAPIResponse = async (
  data: object | WordPressAPIError
  ...
  return eventsData.events.map(event => {
    ...
    const location =
      category === EventCategory.IRL && event.venue ? event.venue.venue : null;
...
export const parseAnnouncements = async (
  data: object | WordPressAPIError
  ...
  const announcementsData = data as {
    announcements: Array<{
      ...
      acf: {
        active: boolean;
        start_time_activity: string;
        end_time_activity: string;
      };
    }>;
  };
...
export const parseShowsAPIData = async (
  data: object | WordPressAPIError
): Promise<Show> => {
  ...
  const show: Show = {
    ...
    postImage:
      typeof showData.acf.post_image === "string"
        ? null
        : Number(showData.acf.post_image),
    mixcloudLink:
      typeof showData.acf.mixcloud_link === "string"
        ? null
        : showData.acf.mixcloud_link.url,
    tags: showData.tags
  };
